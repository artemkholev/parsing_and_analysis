...
class ClassificationPipeline:
    """Pipeline for preprocessing developer classification data."""
    ...
    def filter_it_developers(self) -> None:
    ...
    def create_target_variable(self) -> None:
        """
        Create target variable (junior/middle/senior).
        ...
        def extract_level(row) -> str:
            ...
            experience_years = self._extract_experience_years(experience_text)

            if any(
                keyword in job_title for keyword in ["junior", "джуниор", "младший"]
            ):
                return "junior"
            elif any(
                ...
                keyword in job_title for keyword in ["middle", "миддл", "средний"]
            ):
                return "middle"
    ...
    def extract_features(self) -> None:
        ...
        self.df_developers["age"] = self.df_developers[
            self.df_developers.columns[1]
        ].apply(self._extract_age)

        self.df_developers["salary"] = self.df_developers[
            self.df_developers.columns[2]
        ].apply(self._extract_salary)

        self.df_developers["city"] = self.df_developers[
            self.df_developers.columns[4]
        ].apply(self._extract_city)

        self.df_developers["experience_years"] = self.df_developers[
            self.df_developers.columns[6]
        ].apply(self._extract_experience_years)

        self.df_developers["education"] = self.df_developers[
            self.df_developers.columns[9]
        ].apply(self._extract_education)
        ...
        self.df_developers["has_car"] = self.df_developers[
            self.df_developers.columns[11]
        ].apply(
            lambda x: (
                1
        ...
        # Fill missing values
        self.df_developers["age"].fillna(
            self.df_developers["age"].median(), inplace=True
        )
    ...
    def group_rare_categories(self) -> None:
    ...
    def prepare_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        self.df_developers = self.df_developers[
            self.df_developers["level"].notna()
        ].copy()
